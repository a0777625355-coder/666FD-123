(function () {
  ...
  panel.innerHTML = '<div class="kpl-refresh-row"><strong>赛事近况</strong><button type="button" class="btn btn-ghost btn-sm">↻ 刷新</button></div><p class="kpl-update-status" role="status"></p><p class="kpl-live-copy"></p><div class="kpl-live-sources"></div><small>联网检索摘要，可能存在信息延迟；赛中比分请以赛事中心为准。</small>';
  ...
  async function refresh() {
    if (busy) return;
    busy = true; lastAttempt = Date.now(); button.disabled = true; button.textContent = '更新中…';
    ...
    } finally { busy = false; button.disabled = false; button.textContent = '↻ 刷新'; }
  }
